...
| `docs/session-logs/` | `/blog-reflect` のセッション単位の生メモ（`{YYYY-MM-DD}-{slug}.md`）                                     |
...
- 日付を付けるのは `/blog-polish` で `articles/` に移す公開記事と、`docs/session-logs/` の生メモのみ（`{YYYY-MM-DD}-{slug}.md` 形式）。`ideas/`・`drafts/` など途中段階は日付なし
